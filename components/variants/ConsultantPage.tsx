...
import s from "@/app/design-03/page.module.css";
import h5 from "./Hero05.module.css";
import st from "./ServiceStack.module.css";
import tm from "./Testimonials.module.css";
...
type Props = {
  /** Numeris peržiūros jungikliui */
  current: number;
  /** "light" – originalus 03 hero; "dark" – tamsus hero su nuotrauka iš 05 krypties */
  heroStyle?: "light" | "dark";
  /** "grid" – kortelių tinklelis; "stack" – kortelės kraunasi viena ant kitos slenkant (iš 08) */
  servicesLayout?: "grid" | "stack";
  withTestimonials?: boolean;
};

/** 03 krypties („Moderni konsultanto“) puslapis su keičiamomis dalimis – naudojamas 03, 10 ir 11 kryptyse. */
export default function ConsultantPage({ current, heroStyle = "light", servicesLayout = "grid", withTestimonials = false }: Props) {
  const dark = heroStyle === "dark";
  return (
    <div className={s.page}>
      ...
      <header className={`${s.header} ${dark ? h5.headerDark : ""}`}>
      ...
      <main id="turinys">
        {dark ? (
          /* HERO – tamsus, iš 05 krypties */
          <section id="pagrindinis" className={h5.hero} aria-labelledby="hero-title">
            <div className={h5.heroMedia}>
              <Photo name="portrait" className={h5.heroPhoto} sizes="(min-width: 960px) 55vw, 100vw" priority reveal={false} position="50% 20%" />
            </div>
            <div className={h5.heroText}>
              <p className={h5.kicker}>{brand.role}</p>
              <h1 id="hero-title" className={h5.heroTitle}>
                Investuoti gali <em>kiekvienas.</em>
              </h1>
              <p className={h5.heroLead}>{hero.lead}</p>
              <p className={h5.heroBody}>{hero.body}</p>
              <div className={h5.actions}>
                <a href={cta.href} className={h5.btn}>
                  {cta.intro}
                </a>
                <a href={cta.href} className={h5.btnOutline}>
                  {cta.primary}
                </a>
              </div>
            </div>
            <ul role="list" className={h5.heroCreds} aria-label="Kompetencija">
              {FACTS.map((f) => (
                <li key={f.value}>
                  <span className={h5.credValue}>{f.value}</span>
                  <span className={h5.credLabel}>{f.label}</span>
                </li>
              ))}
            </ul>
          </section>
        ...
        <section id="paslaugos" className={s.section} aria-labelledby="services-title">
          ...
          {servicesLayout === "stack" ? (
            <ul role="list" className={st.stack}>
              {services.map((svc, i) => (
                <li key={svc.id} className={st.card} style={{ ["--i" as string]: i, ["--n" as string]: services.length }}>
                  <div className={st.cardInner}>
                    <p className={st.situation}>„{svc.situation}“</p>
                    <h3 className={st.title}>{svc.title}</h3>
                    <p className={st.text}>{svc.full}</p>
                    <a href={svc.href} className={st.link}>
                      {cta.more}
          ...
          <section className={s.section} aria-labelledby="reviews-title">
            <div className={tm.head}>
              <div>
                ...
                <h2 id="reviews-title" className={s.h2}>
                  Ką sako klientai
                </h2>
              </div>
              {testimonials.length === 0 && (
                <p className={tm.notice}>
                  <span className={tm.badge}>Pavyzdys</span> Tekstai pavyzdiniai – bus pakeisti tikrais klientų atsiliepimais.
                </p>
              )}
            </div>
            <ul role="list" className={tm.grid}>
              {(testimonials.length > 0 ? testimonials : sampleTestimonials).map((t, i) => (
                <li key={i} className={tm.card} data-reveal>
                  {testimonials.length === 0 && <span className={tm.badge}>Pavyzdys</span>}
